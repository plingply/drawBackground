    ...
    let drawCanvas = function() {
        ...
        /*
        	思路：
        		1.双层循环点的集合
        */
        let e, x_dist, y_dist, dist, line_width, i;
        random_points.forEach((item, index) => {
            ...
            for (i = index + 1; i < allPoint.length; i++) {
                ...
                if (!!e.x && !!e.y) {
                    ...
                    dist < e.max && (e == current_point && dist >= e.max / 2 && (item.x -= 0.03 * x_dist, item.y -= 0.03 * y_dist),
                        line_width = (e.max - dist) / e.max,
                        context.beginPath(),
                        context.lineWidth = line_width,
                        context.strokeStyle = "rgba(" + config.color + "," + (line_width + 0.2) + ")",
